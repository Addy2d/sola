...
public class Upgrade1 : MonoBehaviour
{
    ...
    public int increaseShipMagnetSize; // Increase Ship Magnet size by this amount
    ...
    public void OnMouseDown()
    {
        if (canClick)
        {
            ...
            ShipIncreaseMagetUpgrade increaseShipCollider = FindObjectOfType<ShipIncreaseMagetUpgrade>();
            MagnetIncreaseSprite increaseShipSpriteSize = FindObjectOfType<MagnetIncreaseSprite>();

            for (int i = 0; i <= increaseShipMagnetSize; i++)
            {
                increaseShipCollider.IncreaseCircleCollider();
            }
        }
    }
}
